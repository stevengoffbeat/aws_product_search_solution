...
class ProductOpenSearchStack(Stack):
  ...
  def __init__(self, scope: Construct, construct_id: str , **kwargs) -> None:
    ...
    master_user_secret = aws_secretsmanager.Secret(self, "OpenSearchMasterUserSecret",
        ...
        secret_name = "product-opensearch-master-user"
    )
    ...
    cdk.Tags.of(ops_domain).add('Name', 'productsearch-ops')
    ...
    host_url_secret = aws_secretsmanager.Secret(self, "OpenSearchHostURLSecret",
        generate_secret_string=aws_secretsmanager.SecretStringGenerator(
            secret_string_template=json.dumps({"host": self.search_domain_endpoint}),
            generate_string_key="urlkey",
        ),
        secret_name = "product-opensearch-host-url"
    )